...
function register(it, sendTo, adapterShortName, writeNulls, assumeExistingData, additionalActiveObjects) {
    ...
    it(`Test ${adapterShortName}: Read values from DB using GetHistory`, function (done) {
        ...
        sendTo(
            ...
            function (result) {
                ...
                assert.ok(result.result.length >= 4, `Expected at least 4 entries, got ${result.result.length}`);
                var found = 0;
                    ...
                    function (result) {
                        ...
                        sendTo(
                            ...
                            function (result) {
                                ...
                                assert.strictEqual(found, 2);
                                assert.ok(
                                    result.result[0].ts >= latestTs,
                                    `Expected ${result.result[0].ts} >= ${latestTs}`,
                                );
            ...
            function (result) {
                ...
                if (instanceName !== 'influxdb.0') {
                    ...
                    assert.ok(
                        ...
                        `Expected length within 4..5, got ${result.result.length}`,
                    );
                    assert.ok(
                        result.result[1].val >= 1 && result.result[1].val <= 1.5,
                        `Expected ${result.result[1].val} within 1..1.5`,
                    );
                    assert.ok(
                        result.result[2].val >= 2 && result.result[2].val <= 3,
                        `Expected ${result.result[2].val} within 2..3`,
                    );
                    assert.ok(
                        result.result[3].val >= 2 && result.result[3].val <= 3,
                        `Expected ${result.result[3].val} within 2..3`,
                    );
                }
                assert.strictEqual(result.result[0].id, `${instanceName}.testValue`);
                done();
    ...
    it(`Test ${adapterShortName}: Write debounced Raw values into DB`, async function () {
        ...
        return new Promise(resolve => {
            sendTo(
                ...
                function (result) {
                    console.log(JSON.stringify(result.result, null, 2));
                    assert.ok(result.result.length >= 9, `Expected at least 9 entries, got ${result.result.length}`);
    ...
    it(`Test ${adapterShortName}: Write debounced values into DB`, async function () {
        ...
        return new Promise(resolve => {
            sendTo(
                ...
                function (result) {
                    console.log(JSON.stringify(result.result, null, 2));
                    assert.ok(result.result.length >= 12, `Expected at least 12 entries, got ${result.result.length}`);
                    ...
                    for (let i = 0; i < result.result.length; i++) {
                        ...
                        assert.ok(
                            ...
                            `Expected ${result.result[i].val} <= ${expectedVals[expectedId]}`,
                        );
        ...
        sendTo(
            ...
            result => {
                ...
                if (instanceName !== 'influxdb.0') {
                    ...
                    const val = result.result[1] ? result.result[1].val : result.result[0].val;
                    if (process.env.INFLUXDB2) {
                        assert.ok(
                            result.result.length >= 1 && result.result.length <= 3,
                            `Expected length within 1..3, got ${result.result.length}`,
                        );
                        assert.ok(val >= 5 && val <= 7, `Expected ${val} within 5..7`);
                    } else {
                        assert.ok(
                            result.result.length >= 1 && result.result.length <= 2,
                            `Expected length within 1..2, got ${result.result.length}`,
                        );
                        assert.ok(val >= 5 && val <= 7, `Expected ${val} within 5..7`);
                    }
                    assert.strictEqual(result.result[0].id, `${instanceName}.testValueDebounce alias`);
            ...
            function (result) {
                ...
                if (instanceName !== 'influxdb.0') {
                    ...
                    assert.ok(
                        ...
                        `Expected length within 3..5, got ${result.result.length}`,
                    );
                }
                assert.strictEqual(result.result[0].id, `${instanceName}.testValueDebounce alias`);
                done();
            ...
            function (result) {
                ...
                if (instanceName !== 'influxdb.0') {
                    ...
                    assert.ok(
                        ...
                        `Expected length within 3..6, got ${result.result.length}`,
                    );
                }
                assert.strictEqual(result.result[0].id, `${instanceName}.testValueDebounce alias`);
                done();
        ...
        return new Promise(resolve => {
            sendTo(
                ...
                function (result) {
                    ...
                    assert.ok(result.result.length >= 9, `Expected at least 9 entries, got ${result.result.length}`);
                    assert.strictEqual(result.result[0].val, 1);
                    assert.ok(result.result[1].val >= 2.3, `Expected ${result.result[1].val} to be at least 2.3`);
                                ...
                                if (instanceName !== 'influxdb.0') {
                                    ...
                                    if (assumeExistingData) {
                                        assert.ok(
                                            ...
                                            `Expected ${result.result[0].val} within 3700..3755`,
                                        );
                                    } else {
                                        assert.ok(
                                            result.result[0].val >= 3700 && result.result[0].val <= 3800,
                                            `Expected ${result.result[0].val} within 3700..3800`,
                                        );
                                    }
                                } else {
                                    if (assumeExistingData) {
                                        assert.ok(
                                            ...
                                            `Expected length within 2..3, got ${result.result.length}`,
                                        );
                                        if (process.env.INFLUXDB2) {
                                        ...
                                        assert.strictEqual(result.result.length, 2);
                                        if (process.env.INFLUXDB2) {
                                            const sum = result.result[0].val + result.result[1].val;
                                            assert.ok(sum >= 2980 && sum <= 3000, `Expected ${sum} within 2980..3000`);
                                        } else {
                                            assert.strictEqual(
                                        ...
                                        if (instanceName !== 'influxdb.0') {
                                            ...
                                            if (assumeExistingData) {
                                                assert.ok(
                                                    ...
                                                    `Expected ${result.result[0].val} < 62.25`,
                                                );
                                            } else {
                                            ...
                                            if (assumeExistingData) {
                                                ...
                                                if (process.env.INFLUXDB2) {
                                                    ...
                                                    assert.ok(sum >= 49 && sum <= 50, `Expected ${sum} within 49..50`);
                                                } else {
                                                    assert.strictEqual(
                                                        parseFloat(
                                                            (result.result[0].val + result.result[1].val).toFixed(2),
                                        ...
                                        sendTo(
                                            ...
                                            function (result) {
                                                ...
                                                if (instanceName !== 'influxdb.0') {
                                                    ...
                                                    assert.ok(sum >= 30 && sum <= 50, `Expected ${sum} within 30..50`);
                                                }
                                                                        ...
                                                                        if (instanceName !== 'influxdb.0') {
                                                                            ...
                                                                            if (assumeExistingData) {
                                                                                assert.ok(
                                                                                    ...
                                                                                    `Expected ${result.result[0].val} within 31..32`,
                                                                                );
                                                                            } else {
                                                                                assert.ok(
                                                                                    result.result[0].val >= 32 &&
                                                                                        result.result[0].val <= 33.5,
                                                                                    `Expected ${result.result[0].val} within 32..33.5`,
                                                                                );
                                                                            }
                                                                        } else {
                                                                            assert.ok(
                                                                            ...
                                                                            if (process.env.INFLUXDB2) {
                                                                                ...
                                                                                if (assumeExistingData) {
                                                                                    assert.ok(
                                                                                        ...
                                                                                        `Expected ${result.result[0].val} within 31..34`,
                                                                                    );
                                                                                } else {
                                                                                    assert.ok(
                                                                                        result.result[0].val >= 32 &&
                                                                            ...
                                                                            function (result) {
                                                                                ...
                                                                                if (instanceName !== 'influxdb.0') {
                                                                                    ...
                                                                                    assert.ok(
                                                                                        ...
                                                                                        `Expected length within 3..4, got ${result.result.length}`,
                                                                                    );
                                                                                    assert.ok(
                                                                                        result.result[1].val >= 4 &&
                                                                                            result.result[1].val <= 19,
                                                                                        `Expected ${result.result[1].val} within 4..19`,
                                                                                    );
                                                                                }
    ...
    it(`Test ${adapterShortName}: Check for written Null values`, function (done) {
        ...
        sendTo(
            ...
            function (result) {
                console.log(JSON.stringify(result.result, null, 2));
                assert.ok(result.result.length >= 5, `Expected at least 5 entries, got ${result.result.length}`);
                var found = 0;
                for (var i = 0; i < result.result.length; i++) {
    ...
    it(`Test ${adapterShortName}: Check for written Data in general`, function (done) {
        ...
        sendTo(
            ...
            function (result) {
                console.log(JSON.stringify(result.result, null, 2));
                const expectedMin = (writeNulls ? 3 : 0) + (assumeExistingData + 1) * 30;
                assert.ok(
                    result.result.length >= expectedMin,
                    `Expected at least ${expectedMin} entries, got ${result.result.length}`,
                );
    ...
    it(`Test ${adapterShortName}: storeState error for unknown Id with rules parameter`, function (done) {
        ...
        sendTo(
            ...
            function (result) {
                assert.ok(!result.success, `Expected success to be falsy, got ${result.success}`);
                ...
                assert.ok(
                    ...
                    `Unexpected error message: ${result.errors[0]}`,
                );
