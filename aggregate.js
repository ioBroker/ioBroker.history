...
function initAggregate(initialOptions, timeIntervals, log) {
...
function sendResponse(adapter, msg, initialOptions, data, startTime, logId) {
    ...
    if (data[0]) {
        ...
        if (!initialOptions.aggregate ||
            ...
            const options = initAggregate(initialOptions, undefined, log);
            options.result = data;
            step = 0;
        ...
        adapter.sendTo(msg.from, msg.command, { result: [], step: null, error: null, sessionId: initialOptions.sessionId }, msg.callback);
    }
}
