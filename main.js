...
class HistoryAdapter extends adapter_core_1.Adapter {
        ...
        super({
            ...
            objectChange: (id, obj) => {
                ...
                if (customConfig && typeof customConfig === 'object' && customConfig.enabled) {
                    ...
                    if (this.history[id].config?.changesOnly &&
                        this.history[id].config.changesRelogInterval > 0) {
                        this.history[id].relogTimeout = setTimeout(_id => this.reLogHelper(_id), this.history[id].config.changesRelogInterval * 500 * Math.random() +
                            this.history[id].config.changesRelogInterval * 500, id);
                    }
    ...
    async main() {
        ...
        try {
            // create directory
            if (!node_fs_1.default.existsSync(this.config.storeDir)) {
                node_fs_1.default.mkdirSync(this.config.storeDir);
            }
        }
        catch (err) {
    ...
    checkRetention(id) {
        if (this.history[id]?.config?.retention) {
            ...
            if (!this.history[id].lastCheck || dt - this.history[id].lastCheck >= 21600000 /* 6 hours */) {
                ...
                const day = (0, getHistory_1.ts2day)(d.getTime());
                for (let i = 0; i < dayList.length; i++) {
                    if (dayList[i] < day) {
                        const file = (0, getHistory_1.getFilenameForID)(this.config.storeDir, dayList[i], id);
                        if (node_fs_1.default.existsSync(file)) {
                            ...
                            let files;
                            try {
                                files = node_fs_1.default.readdirSync(this.config.storeDir + dayList[i]);
                            }
                            catch {
                                files = [];
                            }
                            if (!files.length) {
                                ...
                                try {
                                    node_fs_1.default.rmdirSync(this.config.storeDir + dayList[i]);
                                }
                                catch (ex) {
    ...
    appendFile(id, states) {
        const day = (0, getHistory_1.ts2day)(states[states.length - 1].ts);
        const file = (0, getHistory_1.getFilenameForID)(this.config.storeDir, day, id);
        let data;
        let i;
        for (i = states.length - 1; i >= 0; i--) {
            ...
            if ((0, getHistory_1.ts2day)(states[i].ts) !== day) {
                break;
            }
        }
        ...
        if (node_fs_1.default.existsSync(file)) {
            try {
                data = JSON.parse(node_fs_1.default.readFileSync(file, 'utf8')).concat(data);
            }
            catch (err) {
        ...
        try {
            // create directory
            if (!node_fs_1.default.existsSync(this.config.storeDir + day)) {
                node_fs_1.default.mkdirSync(this.config.storeDir + day);
            }
    ...
    getOneFileData(dayList, dayStart, dayEnd, id, options, data, addId) {
        ...
        for (let i = 0; i < dayList.length; i++) {
            ...
            if (!isNaN(day) && day >= dayStart && day <= dayEnd) {
                const file = (0, getHistory_1.getFilenameForID)(options.path, day, id);
    ...
    getFileData(options, callback) {
        const dayStart = options.start ? parseInt((0, getHistory_1.ts2day)(options.start), 10) : 0;
        const dayEnd = parseInt((0, getHistory_1.ts2day)(options.end), 10);
        const fileData = [];
        // get list of directories
    ...
    handleGetHistory(msg) {
        ...
        const logDebug = (options.logDebug = !!(this.history[options.id]?.config?.enableDebugLogs || this.config.enableDebugLogs));
        ...
        if ((!options.start && options.count) ||
            ...
            try {
                let gh = node_child_process_1.default.fork(`${__dirname}/lib/getHistory.js`, [JSON.stringify(options)], {
                    silent: false,
                });
    ...
    getDirectories(path) {
        ...
        try {
            return node_fs_1.default.readdirSync(path).filter(file => {
                try {
                    return !file.startsWith('.') && node_fs_1.default.statSync(`${path}/${file}`).isDirectory();
                }
                catch {
                    // ignore entry
                    return false;
    ...
    update(id, state) {
        ...
        if (!found) {
            const dayStr = (0, getHistory_1.ts2day)(state.ts);
            ...
            if (!isNaN(day)) {
                ...
                if (node_fs_1.default.existsSync(file)) {
                    try {
                        const res = JSON.parse(node_fs_1.default.readFileSync(file, 'utf8'));
                        ...
                        if (found) {
                            // save file
                            node_fs_1.default.writeFileSync(file, JSON.stringify(res, null, 2), 'utf8');
                        }
                    }
                    catch (error) {
    ...
    _delete(id, state) {
        ...
        if (!found) {
            ...
            if (state.ts) {
                ...
                let dayEnd;
                if (state.start && state.end) {
                    dayStart = parseInt((0, getHistory_1.ts2day)(state.start), 10);
                    dayEnd = parseInt((0, getHistory_1.ts2day)(state.end), 10);
                }
                else if (state.start) {
                    dayStart = parseInt((0, getHistory_1.ts2day)(state.start), 10);
                    dayEnd = parseInt((0, getHistory_1.ts2day)(Date.now()), 10);
                }
                else if (state.end) {
                    dayStart = 0;
                    dayEnd = parseInt((0, getHistory_1.ts2day)(state.end), 10);
            ...
            files.forEach(entry => {
                try {
                    ...
                    if (found) {
                        // save file
                        if (res.length) {
                            node_fs_1.default.writeFileSync(entry.file, JSON.stringify(res, null, 2), 'utf8');
                        }
                        else {
                            // delete file if no data
                            node_fs_1.default.unlinkSync(entry.file);
                        }
                    }
                }
                catch (error) {
