...
import { beautify } from '@iobroker/aggregate';
import { ts2day, getFilenameForID, getSafeId } from './lib/getHistory';
...
/** Type of a datapoint, as the data browser needs it to interpret an edited value */
...
class HistoryAdapter extends Adapter {
    ...
    readRawEntries(
    ...
    ): { entries: IobDataEntry[]; truncated: boolean } {
        const dayStart = start === undefined ? 0 : parseInt(ts2day(start), 10);
        const dayEnd = end === undefined ? 99999999 : parseInt(ts2day(end), 10);
        const inRange = (ts: number): boolean =>
            (start === undefined || ts >= start) && (end === undefined || ts <= end);

        const byTs = new Map<number, IobDataEntry>();
        let truncated = false;
        ...
        for (const dayStr of dayList) {
            ...
            const file = getFilenameForID(this.config.storeDir, dayStr, id);
            if (!fs.existsSync(file)) {
                continue;
            }
    ...
    getRawEntries(msg: ioBroker.Message): void {
        ...
        const sort: 'asc' | 'desc' = msg.message.sort === 'asc' ? 'asc' : 'desc';

        let entries: IobDataEntry[];
    ...
    async getDatapoints(msg: ioBroker.Message): Promise<void> {
        ...
        for (const id in this.history) {
            knownIds.set(getSafeId(id), id);
        }

        const ids = new Set<string>(Object.keys(this.history));

        for (const day of this.getDirectories(this.config.storeDir)) {
            let files: string[];
            try {
                files = await fs.promises.readdir(`${this.config.storeDir}${day}`);
            } catch (err) {
        ...
        for (const id of Array.from(ids).sort((a, b) => a.localeCompare(b))) {
            ...
            try {
                ...
                const objType = (obj?.common as ioBroker.StateCommon | undefined)?.type;
                if (objType === 'number') {
                    type = 'Number';
