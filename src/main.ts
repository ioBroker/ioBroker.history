...
import { Adapter, getAbsoluteDefaultDataDir, type AdapterOptions } from '@iobroker/adapter-core';
import * as GetHistory from './lib/getHistory';
import * as Aggregate from './lib/aggregate';
import { HistoryAdapterConfig } from './types';
import { InternalHistoryOptions, IobDataEntry } from './lib/types';
...
function formatNumber(value: number | string, defaultValue?: number): number {
...
function formatInt(value: number | string, defaultValue?: number): number {
    if (typeof value === 'number') {
        return value;
    }
    if (typeof value === 'string') {
...
class HistoryAdapter extends Adapter {
    ...
    public constructor(options: Partial<AdapterOptions> = {}) {
        super({
            ...
            objectChange: (id: string, obj: ioBroker.Object | null | undefined) => {
                ...
                const customConfig: CustomStateConfig | undefined = obj?.common?.custom?.[this.namespace];
                ...
                if (obj && customConfig && typeof customConfig === 'object' && customConfig.enabled) {
                    ...
                    if (!this.history[formerAliasId]?.config && !this.subscribeAll) {
                        ...
                        for (const _id in this.history) {
                            if (
                                this.history.hasOwnProperty(_id) &&
                                this.history.hasOwnProperty(this.history[_id].realId)
                            ) {
                    ...
                    const changesRelogInterval = this.history[id].config.changesRelogInterval as number;
                    if (this.history[id].config?.changesOnly && changesRelogInterval && changesRelogInterval > 0) {
                        this.history[id].relogTimeout = setTimeout(
                            ...
                            changesRelogInterval * 500 * Math.random() + changesRelogInterval * 500,
                            id,
                        );
                    }
    ...
    storeCached(isFinishing?: boolean, onlyId?: string): void {
        ...
        for (const id in this.history) {
            if (!this.history.hasOwnProperty(id) || (onlyId !== undefined && onlyId !== id)) {
                continue;
            }
    ...
    finish(callback?: () => void): void {
        if (!this.subscribeAll) {
            for (const _id in this.history) {
                if (this.history.hasOwnProperty(_id)) {
                    this.unsubscribeForeignStates(this.history[_id].realId);
                }
            }
        } else {
    ...
    processStartValues(): void {
        if (this.tasksStart?.length) {
            ...
            if (this.history[task.id]?.config?.changesOnly) {
                this.getForeignState(this.history[task.id].realId, (_err, state) => {
    ...
    parseStateConfig(customConfig: CustomStateConfig) {
        ...
        if (
            ...
            customConfig.changesRelogInterval = this.config.changesRelogInterval as number;
        }
        ...
        if (customConfig.retention && (customConfig.retention as number) <= 604800) {
            (customConfig.retention as number) += 86400;
        }
        ...
        if (customConfig.round !== null && customConfig.round !== undefined && customConfig.round !== '') {
            ...
            if (!isFinite(customConfig.round as number) || (customConfig.round as number) < 0) {
                customConfig.round = this.config.round as number;
            } else {
                customConfig.round = Math.pow(10, customConfig.round as number);
            }
        } else {
    ...
    async main(): Promise<void> {
        ...
        if (this.config.writeNulls === undefined) this.config.writeNulls = true;
        ...
        if (this.config.round !== null && this.config.round !== undefined && this.config.round !== '') {
            ...
            if (!isFinite(this.config.round as number) || (this.config.round as number) < 0) {
        ...
        if (doc?.rows) {
            for (let i = 0, l = doc.rows.length; i < l; i++) {
                if (doc.rows[i].value) {
                    ...
                    const realId = id;
                    const customConfig: CustomStateConfig = doc.rows[i].value[this.namespace];
                    if (customConfig?.aliasId) {
                        this.aliasMap[id] = customConfig.aliasId;
                    ...
                    if (customConfig && typeof customConfig === 'object' && customConfig.enabled !== false) {
                        ...
                        const stateStore: HistoryStateStore = this.history[id];
                        count++;
                        this.log.info(`enabled logging of ${id} (Count=${count}), Alias=${id !== realId}`);
                        this.parseStateConfig(stateStore.config);

                        const changesRelogInterval = stateStore.config.changesRelogInterval as number;
                        if (changesRelogInterval > 0) {
                            stateStore.relogTimeout = setTimeout(
                                _id => this.reLogHelper(_id),
                                changesRelogInterval * 500 * Math.random() + changesRelogInterval * 500,
                                id,
                            );
                ...
                if (this.history[id].state && settings.changesOnly) {
                    ...
                    if (typeof state.val === 'number') {
                        if (
                            ...
                            Math.abs((this.history[id].state.val as number) - state.val) <
                                (settings.changesMinDelta as number)
                        ) {
                            if (!valueUnstable && !settings.disableSkippedValueLogging) {
                                this.history[id].skipped = state;
                            }
        ...
        if (this.history[id]) {
            ...
            if (res) {
                ...
                for (let i = res.length - 1; i >= 0; i--) {
                    ...
                    const resEntry: IobDataEntry = { ...res[i] };
    ...
    handleGetHistory(msg: ioBroker.Message): void {
        ...
        const debugLog = (options.logDebug = !!(
        ...
        if (
            ...
            if (1 || typeof GetHistory === 'undefined') {
                ...
                try {
                    let gh: cp.ChildProcess | null = cp.fork(
                        `${__dirname}/lib/getHistory.js`,
                        [JSON.stringify(options)],
                        { silent: false },
                    );

                    let ghTimeout: ReturnType<typeof setTimeout> | null = setTimeout(() => {
                        try {
                            gh?.kill('SIGINT');
                        } catch (err: unknown) {
                    ...
                    gh.on('message', (rawData: unknown): void => {
                        ...
                        const debugCommand: [command: 'debug', ...string[]] = rawData as [
                            command: 'debug',
                            ...string[],
                        ];
                        ...
                        if (getCacheCommand[0] === 'getCache') {
                            ...
                            this.getCachedData(settings, cacheData => {
                                try {
                                    ...
                                    this.log.info(
                                        `${options.logId} Can not send data to forked process: ${(err as Error).message}`,
                                    );
                                }
                            });
                            ...
                            try {
                                ...
                                this.log.info(
                                    `${options.logId} Can not exit forked process: ${(err as Error).message}`,
                                );
                            }
                            gh = null;
                    ...
                    this.log.info(`${options.logId} Can not use parallel requests: ${(err as Error).message}`);
                }
            } else {
                if (options.logDebug) {
                    options.log = this.log.debug;
                ...
                this.getCachedData(options, cachedData => {
                    GetHistory.aggregation(options, cachedData);
                    const data = GetHistory.response(options) as any;
                    ...
                    if (data[0] === 'response') {
                        ...
                        this.log.error(`${options.logId} Unknown response type: ${data[0]}`);
                    }
                });
            }
        }
    ...
    _delete(id: string, state: { ts?: number; start?: number; end?: number }): boolean {
        ...
        if (!found) {
            const files: { file: string; day: number }[] = [];
            if (state.ts) {
                const dayStr = GetHistory.ts2day(state.ts);
                const day = parseInt(dayStr, 10);
                if (!isNaN(day)) {
    ...
    async storeState(msg: ioBroker.Message): Promise<void> {
        ...
        let successCount = 0;
        const message:
            | { state: IobDataEntry; id: string; rules?: boolean }
            | { state: IobDataEntry; id: string; rules?: boolean }[] = msg.message;
    ...
    getEnabledDPs(msg: ioBroker.Message): void {
        ...
        for (const id in this.history) {
            if (this.history.hasOwnProperty(id) && this.history[id]?.config?.enabled) {
