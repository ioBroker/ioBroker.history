...
function initAggregate(options) {
    ...
    settings.step = 1000; // 1 Step is 1 second
    if (options.step !== null && options.step !== undefined) {
        settings.step = options.step;
    } else {
        settings.step = (settings.end - settings.start) / (options.count || 200);
    }
       ...
       if (data[i].ts < 946681200000) data[i].ts *= 1000;
        ...
        if (index > -1 && index <= settings.maxIndex) {
            ...
            if (settings.aggregate == 'max') {
                ...
                if (settings.result[index].val == null || settings.result[index].val < data[i].val) settings.result[index].val = data[i].val;
