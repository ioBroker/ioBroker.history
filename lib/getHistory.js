    ...
    for (let i = 0; i < dayList.length; i++) {
        ...
        if (!isNaN(day) && day >= dayStart && day <= dayEnd) {
            ...
            if (fs.existsSync(file)) {
                ...
                options.log(`${Date.now()}: getFileData aggregate ${day} (length=${fileData.length})`);
...
function response(options) {
    options.log(`${Date.now()}: Start Response`);
    Aggregate.finishAggregation(options);
    options.log(`${Date.now()}: Send Response`);
    if (typeof module === 'undefined' || !module || !module.parent) {
        process.send && process.send(['response', options.result, options.overallLength, options.step]);
