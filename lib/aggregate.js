...
function finishAggregation(options) {
    if (options.aggregate === 'minmax') {
        ...
        for (let k = 0; k < options.result.length; k++) {
            let indexStartTs = options.start + (k * options.step);
            ...
            for (let kk = 0; kk < options.integralDatapoints[k].length; kk++) {
                const valEndTs = options.integralDatapoints[k][kk + 1] ? options.integralDatapoints[k][kk + 1].ts : indexEndTs;
                let valDuration = valEndTs - options.integralDatapoints[k][kk].ts;
                if (valDuration < 0) {
                    console.log(`Integral: ${k}[${kk}] valDuration negative, ignore ${JSON.stringify(options.integralDatapoints[k][kk])} (vs. ${valEndTs})`)
                    break;
                }
                if (valDuration === 0) {
                ...
                let valStart = options.integralDatapoints[k][kk].val;
                // End value is the next value, or if none, assume "linearity
                let valEnd = options.integralDatapoints[k][kk + 1] ? options.integralDatapoints[k][kk + 1].val : (options.integralDatapoints[k + 1] && options.integralDatapoints[k + 1][0] ? options.integralDatapoints[k + 1][0].val : valStart);
                if (options.integralInterpolation !== 'linear' || valStart === valEnd) {
                    const integralAdd = parseFloat(valStart) * valDuration / options.integralUnit;
                    ...
                    const zeroCrossing = (valStart * valDuration) / (valEnd - valStart);
