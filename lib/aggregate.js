...
function aggregationLogic(data, index, options) {
    ...
    if (options.aggregate === 'max') {
        ...
        console.log(`Quantile ${index}: Add ts= ${data.ts} val=${data.val}`);
    } else if (options.aggregate === 'integral') {
        options.integralDatapoints[index].push(data);
        console.log(`Integral ${index}: Add ts= ${data.ts} val=${data.val}`);
    }
}
...
function beautify(options) {
    console.log(`Beautify: ${options.result.length} results`);
                ...
                if (options.aggregate === 'onchange' || !options.aggregate) {
                    ...
                    if (preFirstValue.ts !== firstTS) {
                        if (firstY !== null) {
                            ...
                            console.log(`interpolate ${y} from ${preFirstValue.val} to ${firstY} as first return value`);
                        } else {
                ...
                if (options.aggregate === 'onchange' || !options.aggregate) {
                    ...
                    if (postLastValue.ts !== lastTS) {
                        ...
                        if (lastY !== null) {
                            ...
                            console.log(`interpolate ${_y} from ${lastY} to ${postLastValue.val} as last return value`);
                        } else {
    ...
    if (options.addId) {
        for (let i = 0; i < options.result.length; i++) {
            if (!options.result[i].id && options.id) {
                console.log(`add id ${options.index || options.id} to result ${i}`);
