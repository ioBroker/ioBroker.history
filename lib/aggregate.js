...
function finishAggregation(options) {
    if (options.aggregate === 'minmax') {
        ...
        for (let k = 0; k < options.result.length; k++) {
            ...
            console.log(`Integral: ${options.integralDatapoints[k].length} datapoints for interval ${k} for ${indexStartTs} - ${indexEndTs}: ${options.integralDatapoints[k].join(',')}`);
            ...
            for (let kk = 0; kk < options.integralDatapoints[k].length; kk++) {
                ...
                if (valDuration < 0) {
                    console.log(`Integral: ${k}[${kk}] valDuration negative, ignore ${JSON.stringify(options.integralDatapoints[k][kk])}`)
                    break;
                }
                if (valDuration === 0) {
