...
// ─── Core functions ──────────────────────────────────────────────────────────
function initAggregate(_options) {
...
function aggregationLogic(data, index, _options) {
    const options = _options;
    let log = () => { };
    if (options.logDebug) {
        log = options.log || console.log;
    ...
    if (options.aggregate === 'max') {
        if (entry.val.val === null || entry.val.val < data.val) {
        ...
        entry.val.val = (entry.val.val ?? 0) + parseFloat(data.val);
...
function aggregation(_options, data) {
    const options = _options;
    let index;
    let preIndex;
...
function finishAggregation(_options) {
    const options = _options;
    let log = () => { };
    if (options.logDebug) {
        log = options.log || console.log;
    ...
    if (options.aggregate === 'minmax') {
        ...
        for (let k = 0; k < options.internalResults.length; k++) {
            ...
            if ((!options.integralDatapoints[k]?.length || options.integralDatapoints[k][0].ts > indexStartTs) &&
            ...
            const point = {
                ts: options.internalResults[k] !== undefined && options.internalResults[k].val.ts
        ...
        if (options.ignoreNull !== false) {
            ...
            if (result[i].val === null) {
                ...
                if (options.ignoreNull === true) {
                    ...
                    result[i].val = options.ignoreNull;
                }
            }
        }
                ...
                if (options.aggregate === 'onchange' || !options.aggregate) {
                    ...
                    if (postLastValue.ts !== lastTS) {
                        ...
                        if (lastY !== null) {
                            // make interpolation
                            const _y = lastY +
                                ((postLastValue.val - lastY) * (options.end - lastTS)) /
    ...
    if (options.addId) {
        for (let i = 0; i < result.length; i++) {
            if (!result[i].id && options.id) {
                result[i].id = options.index || options.id;
            }
        }
    }
...
function sendResponse(adapter, msg, _options, data, startTime) {
    const options = _options;
    let aggregateData;
...
function sendResponseCounter(adapter, msg, _options, data, _startTime) {
    const options = _options;
    // data
    // 1586713810000	100
    // 1586713810010	200
