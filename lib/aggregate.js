...
function aggregation(options, data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (options.aggregate === 'max') {
            ...
            options.quantileDatapoints[index].push(data[i].val);
            console.log(`Quantile ${index}: Add ts= ${data[i].ts} val=${data[i].val}`);
...
function finishAggregation(options) {
    if (options.aggregate === 'minmax') {
        ...
        for (let k = options.result.length - 1; k >= 0; k--) {
            console.log(`Quantile: ${k} ${JSON.stringify(options.result[k].val)} for ${options.quantileDatapoints[k].length} datapoints`);
            if (options.result[k].val.ts) {
                options.result[k] = {
                    ts:  options.result[k].val.ts,
                    val: quantile(options.quantile, options.quantileDatapoints[k])
                };
                console.log(`Quantile ${k} ${options.result[k].ts}: ${options.quantileDatapoints[k].join(', ')} -> ${options.result[k].val}`);
            } else {
                // no one value in this interval
                options.result.splice(k, 1);
