...
function calcDiff(oldVal, newVal) {
    // Ynew - Yold
    const diff = newVal.val - oldVal.val;
    let deltaT;
    // (Xnew - Xold) / 3600000 (to hours)
    deltaT = (newVal.ts - oldVal.ts) / 3600000; // ms => hours
...
function aggregationLogic(data, index, options) {
    ...
    if (options.aggregate === 'max') {
        ...
        options.integralDatapoints.push(data);
    }
}
...
function finishAggregation(options) {
    ...
    if (options.aggregate === 'minmax') {
        ...
        for (let ii = startIndex; ii < endIndex; ii++) {
            ...
            } else if (options.result[ii].min.ts === options.result[ii].max.ts) {
                // if just 2 values: start == min == max, end
                if (
                    options.result[ii].start.ts === options.result[ii].min.ts ||
            ...
            } else if (options.result[ii].start.ts === options.result[ii].max.ts) {
            ...
            } else if (options.result[ii].end.ts === options.result[ii].max.ts) {
        ...
        for (let k = startIndex; k < endIndex; k++) {
            if (options.result[k] !== undefined && options.result[k].val.ts) {
                finalResult.push({
                    ...
                    val:
                        options.result[k].val.val !== null
                            ? Math.round((options.result[k].val.val / options.averageCount[k]) * round) / round
                            : null,
                });
            } else {
        ...
        if (options.integralDatapoints[0] && options.integralDatapoints[0].ts !== options.start) {
            if (
                options.integralDatapoints[0] &&
                options.integralDatapoints[0].ts < options.start &&
                options.integralDatapoints[1] &&
                options.integralDatapoints[1].ts > options.start
            ) {
                const y1 = options.integralDatapoints[0].val;
                const y2 = options.integralDatapoints[1].val;
                const x1 = options.integralDatapoints[0].ts;
                const x2 = options.integralDatapoints[1].ts;
        ...
        const len = options.integralDatapoints.length;
        if (options.integralDatapoints[len - 1] && options.integralDatapoints[len - 1].ts !== options.end) {
            if (
                options.integralDatapoints[len - 1] &&
                options.integralDatapoints[len - 1].ts > options.end &&
                options.integralDatapoints[len - 2] &&
                options.integralDatapoints[len - 2].ts < options.end
            ) {
                const y1 = options.integralDatapoints[len - 2].val;
                const y2 = options.integralDatapoints[len - 1].val;
                const x1 = options.integralDatapoints[len - 2].ts;
                const x2 = options.integralDatapoints[len - 1].ts;
                const val = y1 + ((y2 - y1) * (options.start - x1)) / (x2 - x1);
                options.integralDatapoints[len - 1] = {
                    ts: options.end,
                    val,
                };
            } else if (options.integralDatapoints[len - 1] && options.integralDatapoints[len - 1].ts < options.end) {
                // assume that now we have the same value as before
                options.integralDatapoints.push({
                    ts: options.end,
                    val: options.integralDatapoints[len - 1].val,
                });
            }
        }
        let integral = 0;
        for (let i = 1; i < options.integralDatapoints.length; i++) {
            integral += calcDiff(options.integralDatapoints[i - 1], options.integralDatapoints[i]).square;
        }
        options.result = [
            {
                ts: options.end,
...
function sendResponseCounter(adapter, msg, options, data, startTime) {
    ...
    if (data[0] && data[1]) {
        ...
        if (data[0].ts < options.start) {
            const val =
                data[0].val + (data[1].val - data[0].val) * ((options.start - data[0].ts) / (data[1].ts - data[0].ts));
            data.splice(0, 1);
...
module.exports.calcDiff = calcDiff;
