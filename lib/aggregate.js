...
function finishAggregation(options) {
    if (options.aggregate === 'minmax') {
        ...
        for (var k = options.result.length - 1; k >= 0; k--) {
            console.log('res k=' + k + ': ' + options.result[k].val.ts);
            if (options.result[k].val.ts) {
                ...
                console.log('res2 k=' + k + ': ' + JSON.stringify(options.result[k]));
            } else {
                // no one value in this interval
                options.result.splice(k, 1);
                console.log('removed k=' + k);
            }
        }
    } else {
    ...
    console.log('after finish: ' + JSON.stringify(options));
    beautify(options);
}
...
function beautify(options) {
    ...
    for (var i = 0; i < options.result.length; i++) {
        if (options.ignoreNull !== false) {
            ...
            if (options.result[i].val === null) {
                ...
                if (options.ignoreNull === true) {
                    ...
                    console.log('beautify: remove null ' + i);
                    i--;
                    continue;
                } else {
        ...
        if (options.result[i].ts < options.start) {
            ...
            options.result.splice(i, 1);
            console.log('beautify: remove before start ' + i);
            i--;
            continue;
        }
        ...
        if (options.result[i].ts > options.end) {
            ...
            console.log('beautify: remove after end ' + i);
            break;
        }
    }

    console.log('after start/end check: ' + JSON.stringify(options));
    ...
    console.log('end beautify: ' + JSON.stringify(options));
