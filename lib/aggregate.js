...
function aggregation(options, data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (options.aggregate === 'max') {
            ...
            // try to find best borders ... not accurate but maybe best for now
            const valStartTs = data[i].ts < options.start ? options.start : data[i].ts;
            if (valStartTs === options.start && data[i + 1] && data[i + 1].ts < options.start) {
                console.log(`Integral: Value ${i} is before start and next too, ignore`)
                continue; // this value is before start and next value too, ignore this one
            }
            const valEndTs = data[i + 1] ? data[i + 1].ts : options.end;
            if (valEndTs > options.end) {
                console.log(`Integral: Value ${i} is after end, we are done`)
                break;
            }
            let valDuration = valEndTs - valStartTs;
            let indexEndTs = options.start + ((index + 1) * options.step);
            let partIndex = index;
            let partDuration = indexEndTs - data[i].ts;
            while (data[i].ts + valDuration > indexEndTs) {
                if (!options.result[partIndex].val.ts) {
                    options.result[partIndex].val.ts = Math.round(options.start + (((partIndex - 1) + 0.5) * options.step));
                }
                console.log(`Integral: Value ${i} is longer than index ${index}, extend to next index`)
                console.log(`Integral: ${i} for ${partIndex}: Add ${parseFloat(data[i].val) * partDuration / options.integralUnit} from val=${data[i].val} for ${partDuration}`);
                options.result[partIndex].val.val += parseFloat(data[i].val) * partDuration / options.integralUnit;
                valDuration -= partDuration;
                partIndex++;
                indexEndTs = options.start + ((partIndex + 1) * options.step);
            }
            if (!options.result[partIndex].val.ts) {
                options.result[partIndex].val.ts = Math.round(options.start + (((partIndex - 1) + 0.5) * options.step));
            }
            options.result[partIndex].val.val += parseFloat(data[i].val) * partDuration / options.integralUnit;
            console.log(`Integral: ${i} for ${partIndex}: Add Rest ${parseFloat(data[i].val) * partDuration / options.integralUnit} from val=${data[i].val} for ${partDuration}`);
        }
    }
