...
function initAggregate(options) {
    ...
    options.percentileDatapoints = [];
    options.integralDatapoints = [];
    options.aggregate      = options.aggregate || 'minmax';
    options.overallLength  = 0;

    if (options.aggregate === 'percentile' && (typeof options.percentile !== 'number' || options.percentile <= 0 || options.percentile > 100)) {
    ...
    console.log(`maxIndex = ${options.maxIndex}, step = ${options.step}, start = ${options.start}, end = ${options.end}`);
    ...
    for (let i = 0; i <= options.maxIndex + 2; i++) {
        ...
        if (options.aggregate === 'percentile') {
            options.percentileDatapoints[i] = [];
        }
        if (options.aggregate === 'integral') {
...
function aggregation(options, data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (preIndex < 0) {
            index = 0;
            // if the data ts is even lower then the "pre interval" ignore it, else we collect all data there
            if (data[i].ts < (options.start + ((index -1) * options.step))) {
                continue;
            }
        } else if (preIndex > options.maxIndex) {
            index = options.maxIndex + 2;
            // if the data ts is even higher then the "post interval" ignore it, else we collect all data there
            if (data[i].ts > (options.start + (index * options.step))) {
                continue;
            }
        } else {
            index = preIndex + 1;
        ...
        } else if (options.aggregate === 'percentile') {
            if (!options.result[index].val.ts) {
                options.result[index].val.ts = Math.round(options.start + (((index - 1) + 0.5) * options.step));
            }
            options.percentileDatapoints[index].push(data[i].val);
            console.log(`Percentile ${index}: Add ts= ${data[i].ts} val=${data[i].val}`);
        } else if (options.aggregate === 'integral') {
            if (!options.result[index].val.ts) {
                options.result[index].val.ts = Math.round(options.start + (((index - 1) + 0.5) * options.step));
            }
            options.integralDatapoints[index].push(data[i]);
            console.log(`Integral ${index}: Add ts= ${data[i].ts} val=${data[i].val}`);
        }
    }
    ...
    if (options.aggregate === 'minmax') {
        ...
        for (let k = 0; k < options.result.length; k++) {
            ...
            if ((!options.integralDatapoints[k].length || options.integralDatapoints[k][0].ts > indexStartTs) && options.integralDatapoints[k - 1] && options.integralDatapoints[k -1][options.integralDatapoints[k - 1].length - 1]) {
                ...
                console.log(`Integral: ${k}: Added start entry for interval with ts=${indexStartTs}, val=${options.integralDatapoints[k][0].val}`);
                ...
                console.log(`Integral: ${k}: Added start entry for interval with ts=${indexStartTs}, val=${options.integralDatapoints[k][0].val} with same value as first point in interval because no former datapoint was found`);
            } else if (options.integralDatapoints[k].length && options.integralDatapoints[k][0].ts < indexStartTs) {
                ...
                if (preFirstIndex !== null) {
                    ...
                    console.log(`Integral: ${k}: Remove ${preFirstIndex + 1} entries and add start entry for interval with ts=${indexStartTs}, val=${options.integralDatapoints[k][0].val}`);
                }
            }

            const vals = options.integralDatapoints[k].map(dp => `[${dp.ts}, ${dp.val}]`);
            console.log(`Integral: ${k}: ${options.integralDatapoints[k].length} datapoints for interval  for ${indexStartTs} - ${indexEndTs}: ${vals.join(',')}`);
            ...
            for (let kk = 0; kk < options.integralDatapoints[k].length; kk++) {
                ...
                if (valDuration < 0) {
                    console.log(`Integral: ${k}[${kk}] data do not belong to this interval, ignore ${JSON.stringify(options.integralDatapoints[k][kk])} (vs. ${valEndTs})`)
                    break;
                }
                if (valDuration === 0) {
                    console.log(`Integral: ${k}[${kk}] valDuration zero, ignore ${JSON.stringify(options.integralDatapoints[k][kk])}`)
                    continue;
                }
                ...
                if (options.integralInterpolation !== 'linear' || valStart === valEnd) {
                    ...
                    console.log(`Integral: ${k}[${kk}] : Add ${integralAdd} from val=${valStart} for ${valDuration}`);
                    ...
                    console.log(`Integral: ${k}[${kk}] : Add R${rectPart} + T${trianglePart} => ${integralAdd} from val=${valStart} to ${valEnd} for ${valDuration}`);
                    options.result[k].val.val += integralAdd;
                } else {
                    ...
                    console.log(`Integral: ${k}[${kk}] : Add T${trianglePart1} + T${trianglePart2} => ${integralAdd} from val=${valStart} to ${valEnd} for ${valDuration} (zero crossing ${zeroCrossing})`);
                    options.result[k].val.val += integralAdd;
                }
            }
            options.result[k] = {
        ...
        if (options.removeBorderValues) { // we cut out the additional results
            options.result.splice(0, 1);
            options.percentileDatapoints.splice(0, 1);
            options.result.splice(-1, 1);
            options.percentileDatapoints.splice(-1, 1);
        }
        for (let k = 0; k < options.result.length; k++) {
            if (options.result[k].val.ts) {
                options.result[k] = {
                    ts:  options.result[k].val.ts,
                    val: percentile(options.percentile, options.percentileDatapoints[k])
                };
                console.log(`Percentile ${k} ${options.result[k].ts}: ${options.percentileDatapoints[k].join(', ')} -> ${options.result[k].val}`);
            } else {
                // no one value in this interval
                options.result.splice(k, 1);
            }
