...
function finishAggregationForIntegralEx(options: InternalHistoryOptions): void {
    ...
    // Try to find the very latest value in pre-interval
    let index = 0;
    ...
    for (let i = index; i < options.timeIntervals.length; i++) {
        ...
        if (workDP?.length && current) {
            // calculate first value in this interval
        ...
        // calculate last value in this interval
...
function finishAggregationForIntegral(options: InternalHistoryOptions): void {
    ...
    for (let k = 0; k < options.processing.length; k++) {
        ...
        if (options.timeIntervals) {
            if (k === 0) {
                indexEndTs = options.timeIntervals[0].start;
                indexStartTs =
            ...
            finalResult.push({
                ...
                val:
                    options.processing[k].val.val !== null
                        ? Math.round((options.processing[k].val.val / options.averageCount[k]) * round) / round
                        : null,
            });
        } else {
...
function finishAggregationTotalIntegral(options: InternalHistoryOptions): void {
    ...
    const len = options.totalIntegralDataPoints.length;
    if (options.totalIntegralDataPoints[len - 1] && options.totalIntegralDataPoints[len - 1].ts !== options.end) {
        if (
...
export function sendResponse(
    ...
    if (data[0]) {
        ...
        if (
            ...
            const options: InternalHistoryOptions = initAggregate(initialOptions, null, log);
            options.result = data;
            step = 0;
